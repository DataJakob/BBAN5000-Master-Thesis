...
class PortfolioEnvironment(gym.Env):
    ...
    def __init__(self, stock_data, esg_data, max_steps, window_size=15, objective=None):
        """
...
#     def _calculate_sortino_ratio(self, returns, risk_free_rate=0.0):
#         if len(returns) < 2 or np.isnan(returns).any() or np.isinf(returns).any():
#             return 0.0
#         excess_returns = returns - risk_free_rate
#         downside_returns = excess_returns[excess_returns < 0]
#         if len(downside_returns) == 0:
...
#         average_return = np.mean(returns)
#         max_drawdown = np.max(np.maximum.accumulate(returns) - returns)
#         return average_return / max_drawdown if max_drawdown != 0 else 0.0

#     def _calculate_portfolio_return(self, returns):
#         return np.prod(1 + returns) - 1 if len(returns) > 0 else 0.0

#     def penalised_reward(self, reward, esg_score):
#         reward = 0.0 if np.isnan(reward) or np.isinf(reward) else reward
#         esg_score = 0.0 if np.isnan(esg_score) or np.isinf(esg_score) else esg_score
